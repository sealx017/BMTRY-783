...
import math as math   # offeres math operator functions
from scipy.linalg import solve_triangular # use "from" "import" to load specific functions


# pandas data reading
X = pd.read_csv('/Users/sealso/Research/Teaching/BMTRY_783_2026/Data/wine.data.txt', sep=",", header=None)
X.shape
...
sns_plot1.figure.savefig("/Users/sealso/Research/Teaching/BMTRY_783_2026/Codes/Results/out1.png")
...
sns_plot2.figure.savefig("/Users/sealso/Research/Teaching/BMTRY_783_2026/Codes/Results/out2.png")

sns_plot3 = sns.pairplot(X.loc[:, 'chemical_1':'chemical_5'], kind="hist")
sns_plot3.figure.savefig("//Users/sealso/Research/Teaching/BMTRY_783_2026/Codes/Results/out3.png")

sns_plot4 = sns.pairplot(X.loc[:, 'chemical_1':'chemical_5'], kind="kde")
sns_plot4.figure.savefig("/Users/sealso/Research/Teaching/BMTRY_783_2026/Codes/Results/out4.png")

sns_plot5 = sns.pairplot(X.loc[:, 'cultivar':'chemical_5'], hue="cultivar", kind="kde")
sns_plot5.figure.savefig("/Users/sealso/Research/Teaching/BMTRY_783_2026/Codes/Results/out5.png")
...
result_sigma = sp.optimize.minimize(MVN_sigma, initial_guess,  
...
residual = data - mu
off_diag = np.tril_indices(p, -1)

def theta_to_cholesky(theta):
    L = np.zeros((p, p))

    # Exponentiation guarantees positive diagonal elements
    L[np.diag_indices(p)] = np.exp(theta[:p])
    L[off_diag] = theta[p:]

    return L

def MVN_sigma(theta):
    L = theta_to_cholesky(theta)

    # Solve L z_i = x_i - mu without explicitly inverting Sigma
    Z = solve_triangular(
        L,
        residual.T,
        lower=True,
...
# Small stabilization in case Sigma_initial is nearly singular
scale = max(np.trace(Sigma_initial) / p, 1.0)
Sigma_initial += 1e-8 * scale * np.eye(p)

L_initial = np.linalg.cholesky(Sigma_initial)

initial_theta = np.concatenate([
    np.log(np.diag(L_initial)),
    L_initial[off_diag]
])

result_sigma = sp.optimize.minimize(
    MVN_sigma,
    initial_theta,
    method="L-BFGS-B",
    options={
...
L_hat = theta_to_cholesky(result_sigma.x)
Sigma_hat_optimized = L_hat @ L_hat.T

print(result_sigma)
print(Sigma_hat_optimized)
print(np.linalg.eigvalsh(Sigma_hat_optimized))
...
T2 = n1*n2/(n1 + n2)*np.dot((mean_1 - mean_2).T, 
           np.dot(np.linalg.inv(S_pl), (mean_1-mean_2)))
